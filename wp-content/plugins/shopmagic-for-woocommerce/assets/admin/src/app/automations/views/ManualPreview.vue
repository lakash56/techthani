...
async function fetchPreviewRuns() {
  const CONCURRENT_REQS = 5;
  const batchSize = Math.min(CONCURRENT_REQS, maxPages.value - page.value + 1);
  const promises = Array.from({length: batchSize}, (_, i) => {
    return get(`/automations/${routeParams.id}/manual/matches?page_size=${BATCH_SIZE}&page=${page.value
+ i}`)
  });
  const result = await Promise.all(promises);
  page.value += batchSize;
  for (const matches of result) {
    data.value.push(...matches);
    processed.value++;
  }
}

get<number>(`/automations/${routeParams.id}/manual/max`)
  .then((max) => {
    total.value = max;
  })
  .then(async () => {
